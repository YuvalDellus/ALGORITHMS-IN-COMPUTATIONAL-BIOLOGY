...
def generateEmission(initial_emission):
    ...
    The emmision table will also contain the states of start, end and non-motif state.
    ...
    initial_matrix = np.genfromtxt(fname=f, delimiter="\t", skip_header=1, filling_values=1)
    ...
    row, column = 2, 0
    finalEmission[row: row + initial_matrix.shape[0], column: column + initial_matrix.shape[1]] += initial_matrix
...
def generateTransition(k, p, q):
    ...
    '''
    # Generating new transition matrix in size of the motif plus start, end, and two non-tif states
...
def generateViterbiSeq(scoreMatrix, traceMatrix, k, seq):
    ...
    for i in range(len(seq) - 1, 1, -1):
        if (traceMatrix[maxIndex][i] < 2) or (traceMatrix[maxIndex][i] >= (k + 2)):
            motifSeq += "B"
        else:
            motifSeq += "M"
...
def viterbiAlg(emissionTable, transitionTable, k, seq):
    ...
    traceMatrix = np.zeros((k + 4, len(seq)))  # the Trace matrix will used to find the best trail.
    scoreMatrix[0][0] = 1
    ...
    for nucleotide in range(1, len(seq)):
        ...
        emissionVec = np.transpose(emissionTable)[nucleotideDict[seq[nucleotide]]]
        scoreMatrix[:, nucleotide] = np.max(np.add(scoreVec, np.transpose(transitionTable)), axis=1) + emissionVec
        traceMatrix[:, nucleotide] = np.argmax(np.add(scoreVec, np.transpose(transitionTable)), axis=1)
...
def forwardAlg(emissionTable, transitionTable, k, seq):
    ...
    for nucleotide in range(1, len(seq)):
        scoreVec = np.reshape(scoreMatrix[:, nucleotide - 1], k + 4)
        emissionVec = np.transpose(emissionTable)[nucleotideDict[seq[nucleotide]]]
        finalScore = logsumexp(np.add(scoreVec, np.transpose(transitionTable)), axis=1) + emissionVec  # will use adding insted of multiplication cause using logaritmic rules.
...
def backwardAlg(emissionTable, transitionTable, k, seq):
    ...
    for nucleotide in range(len(seq) - 1, 0, -1):
        ...
        emissionVec = np.transpose(emissionTable)[nucleotideDict[seq[nucleotide - 1]]]
        finalScore = logsumexp(np.add(scoreVec, transitionTable), axis=1) + emissionVec
